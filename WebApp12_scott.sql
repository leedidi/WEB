...
--○ 전체 리스트 조회 쿼리문 구성 → 변경 → LEFT JOIN → 변경 → NVL
SELECT M.SID, M.NAME, M.TEL
    , NVL(S.KOR, -1) AS KOR
    , NVL(S.ENG, -1) AS ENG
    , NVL(S.MAT, -1) AS MAT
...
--○ 생성한 뷰(VIEW_MEMBERSCORE를 활용한 리스트 조회
...
WHERE SID=1;
--> 한 줄 구성
SELECT COUNT(*) FROM TBL_MEMBERSCORE WHERE SID=1
;
--==>> 1
--@ 회원정보 삭제 전에 삭제 가능여부 보기 위해.... 0개인 경우 삭제 가능!
--@ 자식 레코드가 있으면 부모 레코드 삭제 불가능.

--@ 멤버와, 스코어 같이 하나의 어플리케이션 만들겠다...
--@ 이제 입력도, 수정도, 삭제도 할 수 있게 하겠다.... 
