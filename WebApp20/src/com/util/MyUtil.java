...
public class MyUtil
{
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		*/ 
		...
		//   『...?』 상태 또는 『...?...&』 인 상태이다.
		//   이로 인해 결과는 
		//   『...?pageNum=1』 이거나 『...&page=1』가 되는 상황이다.
		//@ '' 페어 주의!
		...
		//@ numPerBlcok = 10! 10개씩 보여주겠다...
		if ( (totalPage>numPerBlock) && (currentPageSetup>0) ) //@  currentPageSetup>0: 이미 페이지가 10 이상!
		{
		...
		//@ currentPageSetup 은 앞 빨간 숫자!(0, 10, 20...) 여기부터 1씩 증가하며 돌아야함
		//-- 『+1』을 수행하는 이유는
		//    앞에서 currentPageSetup 에서 10을 가져왔다면
		//    10 부터 시작하는 것이 아니라
		//    바로가기 페이지는 11부터 시작해야 하기 때문이다.
		
		//@ page<=totalPage : 페이지 53페이지까지만 그리도록
		//@ page<=currentPageSetup+numPerBlock : 페이지가 현재페이지+(10,20,30)보다 작을 때(10번 돌아감)
		while ( (page<=totalPage) && (page<=currentPageSetup+numPerBlock))
		{
			if(page==currentPage)
			{
				strList.append(" <span style='color:orange; font-weight:bold;'>" + page + "</span>");
				//@ 현재 내가 머물고 있는 페이지는 링크가 없어야 함
				//@ 반복문 돌며 딱 한번 들어감! 전체코드는 10번 돌아감
			}
			else
			{
